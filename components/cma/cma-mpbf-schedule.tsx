  ...
  return (
    <>
      <ScheduleLayout title="Schedule 5 — Working Capital Assessment" historical={historical} projections={projections}>
        <ScheduleRow label="Total Current Assets" historicalValue={historical ? formatValue(historical.cash + historical.stock + historical.debtors + historical.otherCurrentAssets) : '-'} projectedValues={projections.map(p => p.totalCurrentAssets)} />
        <ScheduleRow label="Less: Current Liab. (Excl. Bank)" historicalValue={historical ? formatValue(historical.creditors + historical.otherCurrentLiabilities) : '-'} projectedValues={projections.map(p => p.currentLiabilitiesExclBank)} />
        <ScheduleRow label="Working Capital Gap (WCG)" isSubTotal={true} historicalValue={historical ? formatValue(histWCG) : '-'} projectedValues={projections.map(p => p.workingCapitalGap)} />
        <ScheduleRow label="Borrower's Contribution (25% of TCA)" historicalValue={historical ? formatValue((historical.cash + historical.stock + historical.debtors + historical.otherCurrentAssets) * 0.25) : '-'} projectedValues={projections.map(p => p.borrowersContribution)} />
      ...
      <ScheduleLayout title="Schedule 6 — MPBF / Bank Finance Limits" historical={historical} projections={projections}>
        <ScheduleRow label="Working Capital Gap" historicalValue={historical ? formatValue(histWCG) : '-'} projectedValues={projections.map(p => p.workingCapitalGap)} />
        ...
        <ScheduleRow label="Projected CC Utilization" isSubTotal={true} historicalValue={historical ? formatValue(historical.shortTermBorrowings) : '-'} projectedValues={projections.map(p => p.shortTermBorrowings)} />
