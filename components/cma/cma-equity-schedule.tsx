...
export function CMAEquitySchedule({ historical, projections }: CMAScheduleProps) {
  return (
    <ScheduleLayout title="Schedule 13 — Equity / Retained Earnings" historical={historical} projections={projections}>
      ...
      <ScheduleRow label="Closing Equity / Retained Earnings" isSubTotal={true} historicalValue={historical ? formatValue(historical.equity) : '-'} projectedValues={projections.map(p => p.equity)} />
    </ScheduleLayout>
  )
}
